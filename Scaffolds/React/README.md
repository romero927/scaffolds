#Kyle Notes
This scaffold has the following:
- React
- React Router
- Redux
- Redux Thunk
...
- "scripts": 
  {
    "start": "node scripts/start.js", <-- Start with Webpack Dev Server
    "build": "node scripts/build.js", <-- Build with Webpack for Production
    "test": "node scripts/test.js --env=jsdom", <-- Test with Jest 
    "express": "yarn build && node server", <-- Host via Express http://localhost:9000
    "pm2": "pm2 start server", <-- Host via PM2 + Express http://localhost:9000
    "serve": "yarn build && serve -s build", <-- Host via Serve http://localhost:5000
    "docker-build": "docker build -t 'Scaffold' .", <-- Build docker image
    "docker-run": "docker run -p 49160:8080 -d 'Scaffold'" <-- Run docker container
  }
